...
const mobileSchema = z.string().regex(/^\d{10}$/, "Please enter a valid 10-digit mobile number");
  ...
  const validateForm = () => {
    ...
    if (!isLogin) {
      try {
        ...
        newErrors.mobile = e.errors?.[0]?.message;
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = isLogin
        ? { email, password }
        : { email, password, name: fullName.trim(), mobile };
